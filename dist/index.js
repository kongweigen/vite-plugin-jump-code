...
function getArgumentsForLineNumber(editor, fileName, lineNumber, colNumber, workspace) {
  const editorBasename = import_path.default.basename(editor).replace(/\.(exe|cmd|bat)$/i, "");
...
function guessEditor() {
  if (process.env.REACT_EDITOR) {
    return import_shell_quote.default.parse(process.env.REACT_EDITOR);
  }
  try {
    if (process.platform === "darwin") {
      const output = import_child_process.default.execSync("ps x").toString();
      ...
      for (let i = 0; i < runningProcesses.length; i++) {
        ...
        const processName = import_path.default.basename(processPath);
        if (COMMON_EDITORS_WIN.indexOf(processName) !== -1) {
          return [processPath];
        }
      }
    } else if (process.platform === "linux") {
      const output = import_child_process.default.execSync("ps x --no-heading -o comm --sort=comm").toString();
...
function printInstructions(fileName, errorMessage) {
  console.log();
  console.log("Could not open " + import_path.default.basename(fileName) + " in the editor.");
  if (errorMessage) {
...
function launchEditor(fileName, lineNumber, colNumber) {
  ...
  if (process.platform === "linux" && fileName.startsWith("/mnt/") && /Microsoft/i.test(import_os.default.release())) {
    fileName = import_path.default.relative("", fileName);
  }
...
var open_editor_default = launchEditor;
...
function middleware(req, res, next) {
  ...
  const queryParams = import_url.default.parse(req.url, true);
  let path3 = queryParams.query.path;
  let pathList = path3.split(":");
  let rowIndex = pathList[pathList.length - 1];
  path3 = cwd + pathList[0];
  try {
    open_editor_default(path3, Number(rowIndex));
    res.statusCode = 200;
    res.end("ok");
  } catch (error) {
...
function generate(code, filePath) {
...
function transform(code, filePath) {
  let ext = import_path2.default.extname(filePath);
  switch (ext) {
    case ".vue":
        ...
        return [
          {
            injectTo: "head",
            tag: "script",
            children: inject_default
          }
        ];
      }
    },
    transform(code, path3) {
      return transform_default(code, path3);
    }
  };
}
